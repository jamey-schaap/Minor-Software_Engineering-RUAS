...
## Exercises
Remember to implement the `Fun` datatype defined above.
1. Implement the function 
```typescript
incr: Fun<number, number>
``` 
that adds 1 to a number taken as input.

2. Implement the function 
```typescript
isPositive: Fun<number, boolean>
``` 
that checks if a number is positive.

3. Implement the function 
```typescript
safeSqrt: Fun<number, number>
``` 
that takes a number as input. If it is positive it computes the its square root, otherwise it will first turn it into a positive number and then calculate the square root.

4. Implement a function 
```typescript
conditional: (condition: Fun<a, boolean>, _then: Fun<a, b>, _else: Fun<a, b>) => Fun<a, b>
``` 
that returns the function `_then` if the condition returns true, otherwise it returns `_else`.

5. Extend the type `Fun<a, b>` by adding a method 
```typescript
repeat: (this: Fun<a, a>, times: number) => Fun<a, a>
``` 
that repeats (i.e. performs the composition) a function a number of time specified in `times`.

6. Extend the type `Fun<a, b>` by adding a method 
```typescript
repeatUntil: (this: Fun<a, a>, condition: Fun<a, boolean>) => Fun<a, a>
```
that repeats a function until the condition on the input becomes true.
